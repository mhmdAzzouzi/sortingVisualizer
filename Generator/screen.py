...
class Screen(tk.Tk):
        ...
        def _main_frame_1(self):
            ...
            #  self._image_label(tkimage , 1, 0, self.frm )
             ...
             self._create_label(
                 "Generate Your license key and press save", 2, 0, self.frm)
             self._create_entry(3, 0)
        ...
        def _generate_button(self, text, row, column):
            button = tk.Button(self.frm, fg='green', text=text, width=20 , command=lambda: self.controller.generate(self.licenseKey ))
