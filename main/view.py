...
class View(tk.Tk):
    ...
    def _main_frame_1(self):
        ...
        self._create_label(
            "Generate Your license key and press save", 1, 0, self.frm)
  ...
  # message box
    ...
    def _clear_button(self, text, row, column):
        button = tk.Button(self.frm, fg='red',  text=text, width=20,
                           command=lambda: self.controller.clear_text(self.licenseKey))
    ...
    def _verify_button(self, text, row, column):
        button = tk.Button(self.frm, fg='green', text=text, width=20,  command=lambda: self.controller.verify(
            self.licenseKey, self.frm, self.generated_info))
        button.grid(row=row, column=column, padx=5, pady=5)
