...
class View(tk.Tk):
    ...
    def __init__(self, controller):
        ...
        self.main_color ="#291a29"
        self.fonts =  tkFont.Font(family="Comic Sans MS", size=15)
        self.label_fonts = tkFont.Font(family="Comic Sans MS" , size=9)
        #####################
    ...
    def security_frame(self):
        ...
        self.error_label = self.message_label(0,0,self.security_frame,self.errors, "red")
        self.success_label = self.message_label(0,0,self.security_frame,self.success, "green")
    ...
    def user_menu(self):
        self.frm3 = tk.Frame(self.program_frame, width=600, height=200, bg= self.main_color)
    ...
    def draw_frame(self):
        self.drawframe = tk.Frame(self.program_frame, width=600, height=380, bg= self.main_color)
        self.drawframe.grid(row=1, column=0, padx=10, pady=5)
        self.intro = self.intro_label("Press 'Generate' button to generate randomized data" , 0,0 , self.drawframe)
    ...
    def _create_label(self, text, row, column, frame , span):
        label = ttk.Label(frame, text=text , width=15, background= self.main_color, foreground="white")
        label.grid(row=row, column=column,  padx=5, pady=5 , columnspan=span)

    def message_label(self,row,column ,frame, variable, color):
        label= tk.Label(frame , textvariable=variable, fg=color ,background=self.main_color)
        label.grid(row= row, column= column, columnspan=2, pady=10 )
        return label
    def _create_label_frame3(self, text, row, column, frame):
        label = tk.Label(frame, text=text,  background= self.main_color, foreground="white", font=self.label_fonts)
        label.grid(row=row, column=column, padx=5, pady=5, sticky=W)
        return label

    def intro_label(self, text, row, column, frame ):
        label = tk.Label(frame, text=text,  background= self.main_color, foreground="white", font=self.fonts)
    ...
    def _generate_button_3(self, text, row, column, entry1,entry2,entry3):
        button = tk.Button(self.frm3,  background= self.main_color, foreground="white", text=text, font=self.label_fonts,
    ...
    def _start_button(self, text, row, column , speedScale):
        button = tk.Button(self.frm3, font=self.label_fonts, background= self.main_color, foreground="white", text=text , command=lambda: self.sortingAlgo.start_algorithm(speedScale, self.drawframe ))
        button.grid(row=row, column=column)

    def _speed_scale(self, text, row, column):
        scale = tk.Scale( self.frm3 ,  background= self.main_color, foreground="white",  from_=0.09, to=2.0, length=200, digits=2, resolution=0.1, orient=HORIZONTAL, label=text)
    ...
    def _validate_button(self, text, row,  column , entry , entry2):
        button = tk.Button(self.buttons , width=15, font=self.label_fonts, text=text , command= lambda: self.controller.validate(self.program_frame, entry ,entry2 , self.security_frame, self.errors, self.success, self.error_label, self.success_label))
        button.grid(row= row , column=column, padx=5, pady=10)

    def _copy_button(self, text, row, column):
        button = tk.Button(self.buttons ,font=self.label_fonts,   width=14, text=text , command=lambda: self.controller.copy_to_clipbaord(self.serial_num.get(), self.errors , self.success ,  self.error_label, self.success_label))
